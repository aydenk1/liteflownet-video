...
def estimate(model, tenOne, tenTwo):
	#global netNetwork
...
def run(device='0',
	...
	for path, img, im0s, vid_cap in dataset:
        ...
        # Inference
		if(frame_num == 0): #Check if first image
			curr_img = img[0]
			frame_num +=1
			continue
		...
		if save_txt:  # Write to file
			with open(txt_path, 'wb') as f:
				...
				numpy.array([ tenOutput.shape[2], tenOutput.shape[1] ], numpy.int32).tofile(f)
				numpy.array(tenOutput.numpy().transpose(1, 2, 0), numpy.float32).tofile(f)
